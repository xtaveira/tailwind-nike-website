      ...
      <div className="flex justify-between items-start gap-20 flex-wrap max-lg:flex-col">
        ...
        <div className="flex flex-wrap flex-1 justify-between lg:gap-10 gap-20">
              {footerLinks.map((section) => (
                <div key={section}>
                  <h4 className="text-white font-montserrat mb-6 font-medium leading-normal text-2xl">{section.title}</h4>
                  <ul>
                    {section.links.map((link) => (
                      <li key={link.name} className="mt-3 text-white-400 leading-normal hover:text-slate-gray font-montserrat text-base">
                        <a >{link.name}</a>
                      </li>
                    ))}
                  </ul>
